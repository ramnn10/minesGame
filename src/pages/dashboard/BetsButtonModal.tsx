...
const BetsButtonModal: React.FC<BetsButtonModalProps> = ({ isOpen, triggerRef, onClose }) => {
  ...
  return (
    <div className="flex items-center justify-start z-50 w-full">
      <div
        ref={betRef}
        className="w-[290px] bg-[#032e49] border border-[#082130] rounded-lg shadow-xl text-[#c6d6e0]">
        {/* Header */}
        <div className="px-2 py-[3px] flex justify-between items-center">
          <div></div>
          <h2 className="text-[13px] font-[400]">Bet USD</h2>
          <div></div>
          {/* <button
            onClick={onClose}
        ...
        <div className="px-2 py-1 pb-2 text-[13px] grid grid-cols-2 gap-[8px]">
          {Array?.from({ length: 16 })?.map((_, index) => (
            <button
              key={index}
              className="flex justify-center items-center font-[400] hover:bg-[#02578c] cursor-pointer bg-[#094164] border-[1px] rounded-[20px] p-[3px] border-[#00000080]">
              100.00
            </button>
          ))}
        </div>
